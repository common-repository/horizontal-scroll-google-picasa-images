...
if (isset($_POST['hsgpi_form_submit']) && $_POST['hsgpi_form_submit'] == 'yes')
{
	...
	check_admin_referer('hsgpi_form_add');
		...
		$form['hsgpi_googleusername'] = isset($_POST['hsgpi_googleusername']) ? sanitize_text_field($_POST['hsgpi_googleusername']) : '';
	...
	$form['hsgpi_googlealbumid'] = isset($_POST['hsgpi_googlealbumid']) ? sanitize_text_field($_POST['hsgpi_googlealbumid']) : '';
	...
	$form['hsgpi_title'] = isset($_POST['hsgpi_title']) ? sanitize_text_field($_POST['hsgpi_title']) : '';
	if ($form['hsgpi_title'] == '')
	{
	...
	$form['hsgpi_googleimgtype'] = isset($_POST['hsgpi_googleimgtype']) ? sanitize_text_field($_POST['hsgpi_googleimgtype']) : '';
	if($form['hsgpi_googleimgtype'] != "cropped" && $form['hsgpi_googleimgtype'] != "uncropped")
	{
		$form['hsgpi_googleimgtype'] = "cropped";
	}
	
	$form['hsgpi_googleimgcount'] = isset($_POST['hsgpi_googleimgcount']) ? sanitize_text_field($_POST['hsgpi_googleimgcount']) : '';
	$returnvalue = hsgpi_validation::num_val($form['hsgpi_googleimgcount']);
	if ($form['hsgpi_googleimgcount'] == '' || $returnvalue == "invalid")
	{
		$hsgpi_errors[] = __('Number of photos, below 20 is recommended count.', 'horizontal-scroll-google-picasa-images');
		$hsgpi_error_found = TRUE;
	}
	
	$form['hsgpi_fancybox'] = isset($_POST['hsgpi_fancybox']) ? sanitize_text_field($_POST['hsgpi_fancybox']) : '';
	$form['hsgpi_fancybox'] = hsgpi_validation::val_yn($form['hsgpi_fancybox']);
	
	$form['hsgpi_thumbwidth'] = isset($_POST['hsgpi_thumbwidth']) ? sanitize_text_field($_POST['hsgpi_thumbwidth']) : '';
	if(!is_numeric($form['hsgpi_thumbwidth'])) { $form['hsgpi_thumbwidth'] = 200; }
	
	$form['hsgpi_fullwidth'] = isset($_POST['hsgpi_fullwidth']) ? sanitize_text_field($_POST['hsgpi_fullwidth']) : '';
	if(!is_numeric($form['hsgpi_fullwidth'])) { $form['hsgpi_fullwidth'] = 640; }
	
	$form['hsgpi_controls'] = isset($_POST['hsgpi_controls']) ? sanitize_text_field($_POST['hsgpi_controls']) : '';
	$form['hsgpi_controls'] = hsgpi_validation::val_tf($form['hsgpi_controls']);
	
	$form['hsgpi_autointerval'] = isset($_POST['hsgpi_autointerval']) ? sanitize_text_field($_POST['hsgpi_autointerval']) : '';
	$form['hsgpi_autointerval'] = hsgpi_validation::val_tf($form['hsgpi_autointerval']);
	
	$form['hsgpi_intervaltime'] = isset($_POST['hsgpi_intervaltime']) ? sanitize_text_field($_POST['hsgpi_intervaltime']) : '';
	$returnvalue = hsgpi_validation::num_val($form['hsgpi_intervaltime']);
	if ($form['hsgpi_intervaltime'] == '' || $returnvalue == "invalid")
	{
		$hsgpi_errors[] = __('Enter auto interval time in millisecond. (Ex: 1500)', 'horizontal-scroll-google-picasa-images');
		$hsgpi_error_found = TRUE;
	}
	
	$form['hsgpi_animation'] = isset($_POST['hsgpi_animation']) ? sanitize_text_field($_POST['hsgpi_animation']) : '';
	$returnvalue = hsgpi_validation::num_val($form['hsgpi_animation']);
	if ($form['hsgpi_animation'] == '' || $returnvalue == "invalid")
	{
		$hsgpi_errors[] = __('Enter animation duration in millisecond. (Ex: 1000)', 'horizontal-scroll-google-picasa-images');
		$hsgpi_error_found = TRUE;
	}
	
	$form['hsgpi_random'] = isset($_POST['hsgpi_random']) ? sanitize_text_field($_POST['hsgpi_random']) : '';
	$form['hsgpi_arrowcolor'] = isset($_POST['hsgpi_arrowcolor']) ? sanitize_text_field($_POST['hsgpi_arrowcolor']) : '';
	$form['hsgpi_extra2'] = isset($_POST['hsgpi_extra2']) ? sanitize_text_field($_POST['hsgpi_extra2']) : '';
	$form['hsgpi_extra3'] = isset($_POST['hsgpi_extra3']) ? sanitize_text_field($_POST['hsgpi_extra3']) : '';
